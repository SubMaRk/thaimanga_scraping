...
def download_with_retry(url, destination, timeout=5):
    max_retries = 5
    for i in range(max_retries):
        try:
            ...
            # Check if the file exists and if it's the same as expected
            if not check_file_integrity(destination, response):
                print(f"Downloaded file {destination} is not the same as expected. Retrying.")
                continue
...
def check_file_integrity(destination, response):
    ...
    if os.path.exists(destination):
        ...
        # Compare the hashes to check if the file is the same as expected
...
# Create threads for processing manga URLs concurrently
threads = []
with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
    for manga in manga_url:
        thread = executor.submit(process_manga_thread, manga[0])
